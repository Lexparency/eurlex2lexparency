...
class EtlManager:
    ...
    def inform_unavailability(self, celex, version: Version, language):
        ...
        requests.post(
            urljoin(
                LANG_2_ADDRESS[language], f"/_unavailable/eu/{celex}/{version.folder}"
            ),
    ...
    def get_celex_version_list(
        self, language, celex=None, version=None, only_enforced=False, upload=False
    ):
        """See __call__ method"""
        version = Version.create(version) if version is not None else None
        with self.sm() as s:
            q = s.query(Representation).filter(Representation.language == language)
            if version is not None:
                q = q.filter(Representation.date == version.consoli_date)
            if only_enforced:
                q = (
                    q.join(Representation.version)
                    .join(ActVersion.act)
                    .filter(Act.in_force)
                )
            if celex is None:
                if upload:
                    ...
                    q = q.filter(Representation.uploaded.isnot(True)).filter(
                        or_(
    ...
    def remove_transformed(self, celex, language, version: Version = None):
        with self.sm() as s:
            if version is None:
                versions = [
                    Version.create(v.date) for v in s.query(Act).get((celex,)).versions
                ]
            else:
                versions = [version]
            # noinspection PyUnresolvedReferences
            s.execute(
                ...
                .where(Representation.date.in_([v.consoli_date for v in versions]))
                .values(transformation=None)
            )
        for v in versions:
            file_path = os.path.join(
                LEXPATH,
                CelexBase.from_string(celex).path,
                language,
                v.folder,
                "refined.html",
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--rm_local", help="If set, the already existing ", action="store_true"
    )
    parser.add_argument(
        "--upload",
