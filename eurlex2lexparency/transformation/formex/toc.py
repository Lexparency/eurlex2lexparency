...
def get_ordinate_standardizer(language):
    reflector = Reflector(
        language, "annotate", only_treaty_names=True, internet_domain=""
    )
...
class EmbeddedContentsNode(NodeMixin):
    ...
    def _transform(self):
        if self.e.tag == "DIVISION":
            ...
            assert header in (
                self.e[0],
                self.e[1],
            )  # header could be preceeded by a BIB.INSTANCE element.
            self.h1 = header[0]
            self.h2 = header.find("./STI")
            xtml.unfold(header)
            if self.h2 is None:
    ...
    def __repr__(self):
        return (
            '<{} id="{}" ordinate="{}" title="{}">'.format(
                self.e.tag, self.id, self.ordinate, self.title
            )
            .replace(' title="None"', "")
            .replace(' ordinate="None"', "")
            .replace(' id="None"', "")
        )
    ...
    def standardized_ordinate(self):
        if self.e.tag in ("ARTICLE", "ANNEX", "DIVISION"):
            if self.ordinate == "ANNEXES":  # TODO: Adapt to other languages!
                return (
                    "ANX_0"  # Annex-article for Annex-contents table, as in 32017R0745
                )
            try:
    ...
    def id(self):
        if self.e.tag == "LEXP.COMBINED":
            return "toc"
        if (
            self.standardized_ordinate.split("_")[0]
            in ("L", "PRE", "ART", "ANX", "FIN")
            and self.e.tag != "ANNEXES"
        ):
            # self.e.attrib['id'] = self.standardized_ordinate
            return self.standardized_ordinate
        return "toc-{}".format(
            "-".join(node.standardized_ordinate or "" for node in self.path[1:])
        )
