...
class Article(AbstractArticle):
    def __init__(self, element: et.ElementBase, language, transform=False, **kwargs):
    ...
    def _resolve_article_substructure(self):
        while self.source.xpath(".//DIVISION | .//ARTICLE"):
            node = EmbeddedContentsNode(
                self.source.xpath(".//DIVISION | .//ARTICLE")[0], self.language
            )
    ...
    def sub_leaf_titles(self):
        for slt in self.source.xpath(".//TITLE[TI[NP]]"):
            ...
            xtml.unfold(slt[1])
            no_p_text = et.tostring(
                no_p, encoding="unicode", method="text", with_tail=False
            )
    ...
    def _annex_sub_structure(self):
        ...
        for leaf in self.source.xpath(".//article"):
            Article(leaf, language=self.language, logger=self.logger, transform=True)

    def _handle_footnotes(self):
        for k, note in enumerate(self.source.xpath(".//NOTE")):
    ...
    def _final_processing(self):
        for element in self.source.xpath(
            "|".join((".//SIGNATURE ", ".//SIGNATORY ", ".//PL.DATE"))
        ):
            element.attrib[
                "id" if element.tag == "SIGNATURE" else "class"
            ] = element.tag.lower()
            element.tag = "div"
        self.source.attrib["class"] = "lxp-final"
        heading = et.Element("div", attrib={"class": "lxp-heading"})
        et.SubElement(heading, "h1", attrib={"class": "lxp-ordinate"}).text = "Final"
        xtml.push(self.source, heading)

    def _preamble_processing(self):
        for element in self.source.xpath(
            "|".join((".//PREAMBLE.INIT", ".//PREAMBLE.FINAL", ".//GR.CONSID.INIT"))
        ):
    ...
    def _transform_lists(self):
        for list_ in self.source.xpath(
            ".//LIST " "| .//GR.SEQ " "| .//GR.VISA " "| .//GR.CONSID"
        ):
