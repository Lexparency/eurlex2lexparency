...
class HtmlTable(object):
        ...
        for table_part in [self.head, self.body, self.foot]:
            if table_part is not None:
                for row in table_part.iterchildren("tr"):
                    if et.tostring(
                        row[column], method="text", encoding="unicode"
                    ).strip() != "" or row[column].xpath(".//img"):
                        return False
        return True

    def is_equation_array(self):
        if (
            len(self.body.find("tr")) != 3
            or self.head is not None
            or self.foot is not None
        ):
            return False
    ...
    def convert_to_unnumbered_list(self, wise="row"):
        ...
        if wise == "row":
            ...
            columns = (
                self.count_columns()
            )  # TODO: Exist a possibility to use previously calculated values
    ...
    def textify(self, row):
        return "\t".join(
            [
                et.tostring(cell, method="text", encoding="unicode").strip()
    ...
    def convert_pseudo_table_to_list_item(self):
        ...
        active_element.attrib["data-title"] = et.tostring(
            self.body[0][0], method="text", encoding="unicode"
        ).strip(" \n\t\r")
        self.table.getparent().replace(self.table, active_element)


if __name__ == "__main__":
    test_table = et.fromstring(
