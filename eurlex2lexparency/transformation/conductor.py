...
def get_transformer(element: et.ElementBase, language, logger) -> DocumentTransformer:
    ...
    if element.tag == "html":
        ...
        if "alert alert-warning" in class_freq:
            ...
            if "The requested document does not exist." in et.tostring(
                div, encoding="unicode", method="text"
            ):
        ...
        if 1.0 - 2.0 * (p - p_style) / (p + p_style) > 0.9:
            return _OldFashionedConsolidatedAct(element, language, logger=logger)
        try:
            if (
                element.find('./body/p[@class="hd-modifiers"]')
                .text.lower()
                .startswith(repealed_by[language])
            ):
...
def transform(element: et.ElementBase, language, logger=None) -> DocumentTransformer:
    transformer = get_transformer(element, language, logger or logging.getLogger())
...
def get_standardizer(language, fmt="html"):
    if language == "ES":
        n_o = {
            "html": 'n<span class="super">o</span>',
            "fmx": 'n.<HT TYPE="SUP">o</HT>',
        }[fmt]
