...
class AbstractAct:
    ...
    @property
    def languages(self):
        return self.representations[Version.create("initial").consoli_date].keys()
    ...
    def publication_date(self):
        with self.addb() as s:
            r = s.query(Act).filter(Act.celex == self.celex).first().publication_date
        return r
    ...
    def get_language_representations(self):
        with self.addb() as s:
            ...
            consolidates = set([representation.date for representation in act.versions])
        return {
            consoli_date: EurLexLanguagesAndFormats(
                os.path.join(
                    self.local_path, "RDF", Version.create(consoli_date).folder
                ),
                self.celex,
    ...
    def instantiate(self, language, version="latest"):
        ...
        if version.consoli_date not in self.representations:
            raise UnavailableRepresentation(
                "There is no version {} for celex {}".format(str(version), self.celex)
            )
        if language not in self.representations[version.consoli_date]:
            raise UnavailableRepresentation(
                "Version {} for celex {} is not available in language {}.".format(
                    version.folder, self.celex, language
                )
            )
        try:
...
class PhysicalAct:
    def __init__(self, abstract: AbstractAct, language, version):
        ...
        self.local_path = os.path.join(
            self.abstract.local_path, self.language, self.version.folder
        )
        os.makedirs(self.local_path, exist_ok=True)
        self.logger = SwingingFileLogger.get("etl", self.local_path)
        try:
            self.formats = self.abstract.representations[self.version.consoli_date][
                self.language
            ]
        except KeyError:
            raise UnavailableRepresentation(
    ...
    def fall_back_meta_data(self) -> cdm_data.ActMetaData:
        try:
            ...
            type_document = {"R": "REG", "D": "DEC", "L": "DIR"}.get(celex_object.inter)
    ...
    def _instantiate_carefully(self):
        ...
        return SimpleDocument.load(os.path.join(self.local_path, "refined.html"))
    ...
    def _carefully_load(self):
        ...
        try:
            ...
            dl = FormexLoader(
                ...
                if self.version.folder == "initial"
                else "0{}-{}".format(self.abstract.celex[1:], self.version.folder),
    ...
    def _get_meta_data(self) -> cdm_data.ActMetaData:
        ...
        if (
            self.abstract.in_force is not None
    ...
    def get_previous_preamble(self, version: Version):
        ...
        for v in versions:
            ...
            source_ = os.path.join(
                self.abstract.local_path, self.language, v.folder, "refined.html"
            )
            try:
    ...
    def url_html(self, value):
        ...
        self.abstract.representations[self.version.consoli_date][
            self.language
        ] = self.formats
        with self.abstract.addb() as s:
    ...
    def __str__(self):
        return "PhysicalAct(celex={}, language={}, version={})".format(
            self.abstract.celex, self.language, self.version.consoli_date
        )
    ...
    def uploaded(self) -> bool:
        with self.abstract.addb() as s:
            r = s.query(Representation).filter(*self.standard_filters).first().uploaded
        return r
    ...
    def upload(self, address: str):
        ...
        if r.status_code == 200:
            ...
            self.transformation_status = "failed"
            raise UploadError(
                "Something went wrong on uploading: "
