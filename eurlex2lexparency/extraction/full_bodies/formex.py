    ...
    def retrieve(self):
        ...
        if r.status_code != 200:
            raise FormatNotAvailable(
                'Reason "{}" ({})'.format(r.reason, r.status_code))
...
class FormexLoader(Retriever):
    ...
    def __init__(self, local_path, url, language, logger=None):
        self.zipped_formex = BasicFormexLoader(
            local_path, url, language, logger).document
        super().__init__(local_path, url)
        self.logger = logger or logging.getLogger()
    ...
    def retrieve(self):
        zf = self.extract()
        overview = [value for name, value in zf.items()
                    if name.endswith('.doc.xml')][0]
        main = zf[overview.xpath('//DOC.MAIN.PUB/REF.PHYS/@FILE')[0]]
        annexes = [zf[name] for name in overview.xpath(
            '//DOC.SUB.PUB[@TYPE="ANNEX"]/REF.PHYS/@FILE')]
        ...
        required_tifs = set(
            combined.xpath('//INCL.ELEMENT[@TYPE="TIFF"]/@FILEREF'))
        ...
        et.ElementTree(element=combined) \
            .write(os.path.join(self.local_path, 'formex.xml'), encoding='utf-8')
        return combined

    def open(self):
        return et.ElementTree(
            file=os.path.join(self.local_path, 'formex.xml')).getroot()
