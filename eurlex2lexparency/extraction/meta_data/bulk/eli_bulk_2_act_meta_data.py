...
from eurlex2lexparency.extraction.meta_data.cdm_data import ActMetaData, TitlesRetriever
...
class EliKraken(SparqlKraken):
    ...
    _aux_mapping = {
        URIRef("http://data.europa.eu/eli/ontology#InForce-inForce"): Literal(
            "true", datatype=URIRef("http://www.w3.org/2001/XMLSchema#boolean")
        ),
        URIRef("http://data.europa.eu/eli/ontology#InForce-notInForce"): Literal(
...
if __name__ == "__main__":
    ...
    for lang, celex in product(("ES",), celexes):
        ...
        with open(
            os.path.join(RESULT_PATH, f"{celex}_{lang}.json"),
            encoding="utf-8",
            mode="w",
        ) as f:
