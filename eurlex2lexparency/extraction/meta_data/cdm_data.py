...
class Resolver:
    ...
    def _get_new_mapping(self):
        graph = local_graph()
        results = graph.query(
            kraken.queries["new_resolver_map"](type=self.type, language=self.language)
        )
...
class Kraken(SparqlKraken):
    ...
    alt_filter = (
        " ?{name} cdm:resource_legal_id_celex ?filter_celex .\n"
        "  FILTER( str(?filter_celex) = '{{celex}}' ) ."
    ).format
...
class ActMetaData(DocumentMetaData):
    ...
    amended_by = DressedAttribute(
        "resource_legal_amends_resource_legal_by", "eli", True
    )
    ...
    completes = DressedAttribute("resource_legal_completes_resource_legal", "eli", True)
    completed_by = DressedAttribute(
        "resource_legal_completes_resource_legal_by", "eli", True
    )
    corrects = DressedAttribute("resource_legal_corrects_resource_legal", "eli", True)
    ...
    repealed_by = DressedAttribute(
        (
            ...
            "resource_legal_repeals_resource_legal_by",
        ),
        "eli",
        True,
    )
    ...
    def from_ELI(cls, id_local, language):
        with open(
            os.path.join(ELI_PATH, f"{id_local}_{language}.json"), encoding="utf-8"
        ) as f:
    ...
    def cached_retrieve(cls, id_local, language, file_path, only_local=False):
        ...
        with open(file_path, mode="w", encoding="utf-8") as f:
            json.dump(amd.to_dict(), f, ensure_ascii=False, indent=2, default=default)
        return amd
    ...
    def retrieve(cls, celex, language):
        ...
        self.id_local = celex
        results = cls.kraken(
            "act_predicate_object", "subject_predicate_act", celex=celex, lang3=lang3
        )
        for p in ("title", "is_about"):
            r = cls.kraken(
                f"act_{p}_lang", celex=celex, language=language.lower(), lang3=lang3
            )
            ...
            if str(property_) == prefixes["owl"] + "sameAs":
                if value.startswith(cls._eli_resource_trunc_cdm):
                    value = str(value).replace(
                        cls._eli_resource_trunc_cdm, cls._eli_resource_trunc_eli
                    )
    ...
    def retrieve_citations(self):
        ...
        for predicate, template in [
            ("cites", "act_cites_title"),
            ("cited_by", "title_cites_act"),
        ]:
            attribute = getattr(self, predicate)
            for celex, title in self.kraken(template, celex=self.id_local, lang3=lang3):
    ...
    def set_title_data(self):
        ...
        if self.id_human == self.id_local or self.id_human is None:
            # noinspection PyTypeChecker
            self.id_human = CelexBase.from_string(self.id_local).human_id(self.language)
    ...
    def enhance_referrers(self):
        ...
        cellar_2_anchor = TitlesRetriever(self.language, combined).get_anchors()
    ...
    def based_on(self, value):
        if type(value) is Anchor:
            ...
            try:
                self._based_on = tmd.cached_retrieve(value, self.language).as_anchor()
            except KeyError:
                return
    ...
    def popularize(self):
        ...
        """
        short_title = PopularTitles().get_short_title(str(self.id_local), self.language)
    ...
    def parse(cls, e: et.ElementBase):
        ...
        for pair in cls._custom_namespace:
            g.bind(*pair)
        g.parse(
            data=et.tostring(e, encoding="unicode"), publicID=default_iri, format="rdfa"
        )
    ...
    def fill_implementeds(self):
        changers = set()
        for change in ("amended_by", "repealed_by", "corrected_by", "completed_by"):
    ...
    def remove_from_citations(self):
        ...
        other_refs = reduce(
            lambda x, y: x | y,
            [
                {a.href for a in getattr(self, attr)}
                for attr in self._referrers
...
class TitlesRetriever:
    ...
    def __init__(self, language, cellar_ids):
        self.lang_3 = (
            country_mapping.get(two=language) if len(language) == 2 else language
        )
    ...
    @property
    def query(self):
        return self.kraken.queries["title_retriever_base"](
            lang_3=self.lang_3, filter=self.filter
        )
    ...
    def get_anchors(self):
        ...
        return {
            str(cellar): Anchor.create(
                id_local.toPython(), to_python(title), self.lang_2
            )
