...
class EurLexDocumentLandingPage(UrlConstructor):
    ...
    def retrieve(self):
        landing_page = et.ElementTree(
            et.fromstring(
                requests.get(self.url).text, parser=et.HTMLParser(encoding="utf-8")
            )
        )
...
class DocumentEliData(UrlConstructor):
    ...
    def retrieve(self):
        ...
        for triple in self.landing_page.xpath(
            ...
            g.add((self.uri_ref(triple.attrib["about"]), self.uri_ref(p), o))
        for triple in self.landing_page.xpath(
            "/html/head/meta" "[@about and @property and @content and @datatype]"
        ):
            g.add(
                (
