...
_pattern_test_data = {
    "32013R0575": CelexCompound(CelexBase(3, 2013, "R", 575, ""), empty_annex),
    ...
    "32013R0575R(01)": CelexCompound(
        CelexBase(3, 2013, "R", 575, ""), Annex(AnnexType.corrigendum, 1)
    ),
    "31972A0722(05)": CelexCompound(CelexBase(3, 1972, "A", 722, "(05)"), empty_annex),
    "31972A0722(05)R(01)": CelexCompound(
        CelexBase(3, 1972, "A", 722, "(05)"), Annex(AnnexType.corrigendum, 1)
    ),
}
...
class TestCelex(unittest.TestCase):
    def test_pattern_parsing(self):
        for key, value in _pattern_test_data.items():
            self.assertEqual(
                str(CelexCompound.from_string(key)), str(value), f"Failed for {key}"
            )
    ...
    def test_parse_consolidates_extra(self):
        self.assertEqual(
            str(_pattern_test_data["02013R0575-20170315"].base), "32013R0575"
        )
