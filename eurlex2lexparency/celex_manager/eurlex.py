...
from eurlex2lexparency.utils.generics import (
    get_fallbacker,
    wait_until_tomorrow,
    get_file_content,
    TwoWay,
        ...
        with session() as s:
            ...
            if self.celex.type in (AnnexType.consolidate, AnnexType.none):
                ...
                for language in self.languages:
                    record = s.query(Representation).get((celex, v_date, language))
                    if record is None:
                        s.add(
                            Representation(celex=celex, date=v_date, language=language)
                        )
                ...
                for language in self.languages:
                    record = s.query(Correpresentation).get((celex, c_number, language))
                    if record is None:
                        s.add(
                            Correpresentation(
                                celex=celex, number=c_number, language=language
                            )
                        )
...
query_templates = {
    "celex_wild_card": "<![CDATA[DN = {0} OR DN-old = {0} ORDER BY DN ASC]]>",  # used
    "consleg_celex_wild_card": "<![CDATA[DTS_SUBDOM = CONSLEG AND BF = {0}* ORDER BY BF ASC]]>",
    "consleg_celex_year": "<![CDATA[DTS_SUBDOM = CONSLEG AND BF = 3* "  # used
    "AND DD_YEAR = {} ORDER BY DD ASC, BF ASC]]>",
    "all_versions_wild_card": "<![CDATA[(DTS_SUBDOM = CONSLEG "
...
class EurLexWebServiceHit(PersistableHit):
    ...
    def in_force(self):
        try:
            return eval(
                self.work.xpath(
                    "./ns2:RESOURCE_LEGAL_IN-FORCE/ns2:VALUE", namespaces=namespaces
                )[0].text.capitalize()
            )
        except IndexError:
            return
    ...
    def celex(self) -> CelexCompound:
        return CelexCompound.from_string(
            self.work.xpath("./ns2:ID_CELEX/ns2:VALUE", namespaces=namespaces)[0].text
        )
    ...
    @property
    def work(self):
        return self.e.xpath("./ns2:content/ns2:NOTICE/ns2:WORK", namespaces=namespaces)[
            0
        ]
...
class DoQueryResult:
    ...
    def __init__(self, response_text, logger=None):
        self._r_tree = et.ElementTree(et.fromstring(response_text, parser=parser))
        self.logger = logger or logging.getLogger()
        try:
            self._search_results = self._r_tree.xpath(
                "/ns1:Envelope/ns1:Body/ns2:searchResults", namespaces=namespaces
            )[0]
        except IndexError:
            ...
            if (
                fail_reason.text == "The maximum number of call is reached "
                "for web service demands"
            ):
                raise RequestLimitReached
            else:
                raise UnexpectedResponse("New fail reason: {}".format(fail_reason.text))
        else:
            self._total_hits = self._search_results.xpath(
                "./ns2:totalhits", namespaces=namespaces
            )[0]
            self._page = self._search_results.xpath(
                "./ns2:page", namespaces=namespaces
            )[0]
            self._page_size = self._search_results.xpath(
                "./ns2:numhits", namespaces=namespaces
            )[0]
            self.hits = dict()
            for hit in map(
                EurLexWebServiceHit,
                self._search_results.xpath("ns2:result", namespaces=namespaces),
            ):
                try:
                    key = str(hit.celex)
    ...
    def search_language(self):
        return self._search_results.xpath("./ns2:language", namespaces=namespaces)[
            0
        ].text
...
class DoQuery:
    ...
    def __call__(self, query, page=1, page_size=max_page_size, search_language="en"):
        ...
        r = self.http.request(
            ...
            body=self.envelope_template.format(
                query=query, page=page, size=page_size, language=search_language
            ),
            headers=self._headers,
        )
        if (r.status, r.reason) not in [(200, "OK"), (500, "Internal Server Error")]:
            raise RuntimeError(
...
class PreLegalContentXmlDataBase:
    ...
    def pull_all_hits(self, query, resume=False):
        ...
        if os.path.isfile(file_path) and resume:
            ...
            start_page = max(
                1, int(float(hit_list.page_size) / self.inquirer.max_page_size)
            )
            hit_list.extend(self.do_query(query, page=start_page))
            start_page += 1
            self.logger.info(
                f"Resuming query <{query}> ({query_hash}) " f"from page {start_page}."
            )
        else:
...
if __name__ == "__main__":
    a = EurLexWebServiceHit(
        os.path.join(LEXPATH, "PRE", "elements", "02015R0848-20180726.xml")
    )
