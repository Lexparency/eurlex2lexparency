...
from eurlex2lexparency.celex_manager.eurlex import (
    country_mapping,
    PersistableHit,
    PreLegalContentXmlDataBase,
    query_templates,
)
...
class CompoundHit(
    namedtuple("Hit", ["celex", "work_date", "languages"]), PersistableHit
):
    in_force = None
...
class LegislationGetter(SparqlKraken):
    ...
    def _iter_acts(self, month: FullMonth, act_types: tuple):
        ...
        for act_type in act_types:
            ...
            self.logger.info(
                f"Obtained {len(hits)} acts " f"for month {month} and type {act_type}."
            )
            result.update(hits)
    ...
    def pull_all_corrigenda_representations(self, celex: CelexBase):
        ...
        for number, day, lang in results:
            CompoundHit(
                CelexCompound.get(celex, number.toPython()),
                day.toPython(),
                {lang_2(lang)},
            ).persist()
    ...
    def _iter_conslegs(self, month: FullMonth, act_types: tuple):
        ...
        self.logger.info(
            f"Obtained {len(hits)} consolidations "
